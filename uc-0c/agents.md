...
enforcement:
  ...
  - "NEVER GUESS THE GROWTH TYPE: If '--growth-type' is missing or not one of the supported types (MoM, YoY), refuse and exit non-zero — never silently pick one."
  - "REFUSAL CONDITIONS: refuse (exit non-zero, no output file) when (a) --ward/--category missing, (b) --growth-type missing or invalid, (c) the requested ward or category does not exist in the dataset (show the valid list instead of guessing), or (d) the requested ward+category combination has no data rows."
